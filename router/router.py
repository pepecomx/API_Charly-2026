...
async def actualiza_contrasena(
    *,
    id_usuario: str = Body(..., alias="_IdUsuario"),
    contrasena_actual: str = Body(..., alias="_contrasenaActual"),
    contrasena_nueva: str = Body(..., alias="_contrasenaNueva"),
):
    ...
    try:
        with engine.connect() as conn: 
            ...
            contrasena_enc=secret_pwd.encrypt(contrasena_nueva.encode("utf-8"))
            ...
            if id!=None:
                ...
                if (contrasena_actual==contrasena_int_s):    
                    contrasena_enc=secret_pwd.encrypt(contrasena_nueva.encode("utf-8")) 
                    token=secret_Token.encrypt(contrasena_nueva.encode("utf-8"))
                    #print(contrasena_enc)
                    
                    fecha=expire_date(2)
                    result=conn.execute(usuarios_c.update().values(Contrasena=contrasena_enc).where(usuarios_c.c.IdUsuario == id_usuario))
                    ...
                    writeFile(id_usuario,fecha_y_hora,"actualiza_contrasena:Contrase̱a actualizada")
                    ...
                    writeFile(id_usuario,fecha_y_hora,"actualiza_contrasena:Contrase̱a actual no correcta!!")
                    return JSONResponse(content={"message":"Contrase̱a actual no correcta!!"},status_code=403)
                
            else:
                print("Usuario no encontrado !!")
                writeFile(id_usuario,fecha_y_hora,"actualiza_contrasena:Usuario no encontrado !!")                
                return JSONResponse(content={"message":"Usuario no encontrado!!"},status_code=401)
    except: 
        writeFile(id_usuario,fecha_y_hora,"actualiza_contrasena:Error No identificado!!")                
...
async def actualiza_token(
    *,
    id_usuario: str = Body(..., alias="_IdUsuario"),
    contrasena: str = Body(..., alias="_contrasena"),
):
#async def obtiene_token(_IdUsuario: str ,_contrasena: str ):     
        ...
        with engine.connect() as conn: 
            ...
            if id!=None:
                ...
                if contrasena==contrasena_int_s:          
                    ...
                    #    return datos[3]
                    #else :
                    token=secret_Token.encrypt(contrasena.encode("utf-8"))                        
                    fecha=expire_date(2)
